...
be merged in  the  future, although this  will  be hard  work. This document corresponds to the release
version 2.2, which can be found in the branch RELEASE-2.2-DL1YCF of the repository.
...
If you are running two receivers, it depends on the audio output module whether
...
\clearpage
\textbf{\color{red}Virtual audio cables, PulseAudio case (Linux only).}\\
...
So we shall create two dummy ("null-sink") output devices which
we shall call RXcable and TXcable. This is done by the commands (do not enter the line breaks)
...
search the internet on how to do this.
...
\textit{is} active. Here we give a list of available options. For each of these, we note if it
is enabled or disabled in the \texttt{Makefile} of the standard repository.
...
\rett{ALSA}. (default: disabled)
This option is only effective for LINUX systems including the Raspberry Pi. If this
option is active, local audio output is done via the standard LINUX ALSA library, otherwise
PulseAudio is used. Note that sending audio to HDMI monitors is not (yet) supported
with the ALSA option (with PulseAudio, this is no problem).
On MacOS, this option has no effect since the portaudio library is used
in any case.

\rett{EXTNR}. (default: disabled)
