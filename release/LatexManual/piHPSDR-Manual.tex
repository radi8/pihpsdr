...
(here: v2.3 that is, \textit{not} dirty) this means that one or more files have been locally  modified
...
the \rett{Serial Console} button. Note that the built-in serial port uses GPIO lines 14 and 15, so
you should only activate it when you a) need it and b) have checked \rett{No Controller} in the
initial GPIO menu (or have not compiled the program with GPIO support), since GPIO lines 14 and 15 are
used for \texttt{Controller1}, \texttt{Controller2}, and {G2 Front Panel} (see Appendix \ref{sec:gpio}).
