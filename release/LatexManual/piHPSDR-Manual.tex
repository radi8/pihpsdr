...
The second line of control elementes, beginning with the \rett{Connect Server} button, belongs to the
client/server feature that is described in chapter \ref{sec:clientserver}, so
...
allow being discovered by such ``routed'' (non-broadcast) packets.
...
\textbf{Special behavior in SAT/RSAT mode.} Normally (that is, not in \bltt{SAT} or \bltt{RSAT} mode), a memory store
...
illegal use of the transmitter. While the password is stored un-encrypted on the server side, a challenge/response
...
client-server operation. If it is a really long password, only the first 50 characters are used, and if it
...
the pop-down menu and enter a new \texttt{host:addr} pair, which will then be added to the list. {\color{red}
After making changes in the text field, you have to press the \texttt{Enter} key to store the new entry
and make it effective.} The last element of
...
stored in the preferences file so you have to enter it each time you connect. By default, the last button in
...
The password you enter is \textbf{not} transmitted ``as is'' over the line. Instead, the server sends a random
64-byte \textit{challenge} which is different for each connection.
On the client side, a four-byte version number and the password is added
to the challenge and a SHA256 key is built from the result and then sent back to the server. The server
does the same with its version number and the password entered in the \bltt{Server} menu and then asserts
that the two SHA256 keys are identical. This is most likely not top-notch cyber security, but should provide
a decent level to fulfil your  obligations as a radio amateur to prevent your gear from unlawful use.
If you feel you must go beyond, your option is to set up a VPN \textit{virtual private network} in which
you can have cyber security provided by your network infrastructure rather than by an application such 
as \pH.

\textbf{Auto disconnect feature:} Equally important is to prevent that your radio stops transmitting if
...
next command from the client failed (the client will send and empty
``heartbeat'' command every 15 seconds).
...
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{HermesLite-II and RadioBerry support}
