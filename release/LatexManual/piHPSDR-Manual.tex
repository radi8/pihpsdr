...
WDSP library, the
transfer of audio input (e.g. from a microphone) or output (e.g. to a headephone) data,
...
\texttt{Radio} menu. If there are two receivers (called RX1 and RX2),
 ...
 has two RF frontend with two analog-to-digital converters (ADCs),
...
prefer the Peak value here and made it the default in \pH:
...
\begin{tabular}{l|l}
\toprule
VFO step size & step size  \\\hline
Filter & filter type used (including CW peak) \\\hline
Noise reduction & all settings \\\hline
Noise blanker & all settings \\\hline
Automatic notch filter &  on/off \\\hline
Spectral  noise blanker &  on/off \\\hline
...
seen (see Fig. \ref{fig:TenBandEqualizer}). The horizontal
space required to display all the spin buttons an sliders is
quite large (about 900 pixels) such that the use of ten-
...
or less, a four-channel equaliser is usually sufficient and all
controls relevant to the four-channel equalizers are in the left half
of th e menu window. The possibility
...
Note that all corner frequency are strictly
kept in ascending order, so it may happen that when increasing
a corner frequency, frequencies of the higher channels are
increased to keep the order. Likewise, when reducing a corner
frequency, frequencies of the lower channels might get reduced
as well. The allowed range for corner frequencies is 50 to
16000 Hz.

Equaliser settings are saved with the mode, so
if you adjust the equalisers when doing LSB, and then switch to
DIGU, the equaliser settings for DIGU become effective
(normally, equalizers are disabled for digital modes).
They resume their LSB
settings upon switching back to LSB. This also applies to other
...
%Note that on some systems, adjusting the window size when
%switching between four-band and ten-band equalizers took
%very long, so the menu window in four-band mode appears too wide
%since it contains much blank space in its right half, where
%the controls of the bands 5 through 10 are located.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
has those serial interfaces available). Note that on the new ANAN G2MkII radios, one of the serial lines
...
is used on the ANAN G2-MkII radios for communicating with the front panel. If you chose
...
\action{AF Gain}{AFGAIN}{PE}{
{\color{red} This action is connected to the \texttt{AF Gain} slider in the slider area.}
...
\action{AGC Gain}{AGCGain}{PE}{
{\color{red} This action is connected to the \texttt{AGC} slider in the slider area.}
Change AGC gain ($-20\ldots 120$ dB) of the active receiver. Changing the AGC gain moves the
horizontal green line (doted by a green 'G') on the panadapter. Its optimal position is
...
\action{Atten}{ATTEN}{PE}{
{\color{red} This action is connected to the \texttt{ATT} slider in the slider area.}
...
filter is one of the two variable filters
 ...
 \action{Function}{FUNC}{B}{Cycle through the six toolbar sets. For the \pH GPIO Controller1, where
 ...
 of the switches. Note that this action is \textit{always} connected with the right-most toolbar button.}
  ...
  \action{FuncRev}{FUNC-}{B}{Cycle backwards through the six toolbar sets. For the \pH GPIO Controller1,
...
\action{Mic Gain}{MICGAIN}{PE}{
{\color{red} This action is connected to the \texttt{Mic Gain} slider in the slider area.}
...
\action{PanZoom}{PAN}{E}{
{\color{red} This action is connected to the \texttt{Pan} slider in the Zoom/Pan area.}
...
\action{RF Gain}{RFGAIN}{PE}{
{\color{red} This action is connected to the \texttt{RF Gain} slider in the slider area.}
...
\action{RIT}{RIT}{E}{Change the RIT value of the active receiver in the range -9999 to 9999 Hz.
...
\action{RX Menu}{RX}{B}{Opens the \texttt{RX} menu for the active receiver.}
...
\action{Squelch}{SQUELCH}{PE}{
{\color{red} This action is connected to the \texttt{Sqlch} slider in the slider area.}
...
\action{TX Drive}{TXDRV}{PE}{
{\color{red} This action is connected to the \texttt{TX Drv} slider in the slider area.}
...
is set, XIT is
automatically disabled, if a non-zero value ist set, XIT is enabled. Each tick of the encoder changes
...
\action{XIT -}{XIT-}{B}{Decrement the XIT value of the transmitter by 10 times the RIT step size, in the range
-9999 to 9999 Hz. If a value of zero
...
\action{XIT +}{XIT+}{B}{Increment the XIT value of the transmitter  by  10 times the RIT step size, in the range
-9999 to 9999 Hz. If a value of zero
is reached, XIT is automatically disabled, and if a nonzero value is reached, XIT is automatically enabled.
Note that this action belongs to the few ones for which a button release event has an effect. If you press
and hold XIT+ (either
...
\action{Zoom}{ZOOM}{PE}{
{\color{red} This action is connected to the \texttt{Zoom} slider in the Zoom/Pan area.}
...
\textit{the} multi-function encoder by assigning the \bltt{Multi} action (For the G2 Mk2, the assignment
is fixed, as for all other encoders an push-buttos there).
...
\section{Accessability for the Blind}
...
command-line options fine-tuning its behavior (which are not used by the \texttt{udplistener}
program since it is simply a demo), but I have not seen how to customize \texttt{espeak-ng}
...
$\phantom{\Big|}${\large Notes} & \multicolumn{1}{|p{11cm}|}{ANDROMEDA extension. p$_1$ = number of VFO steps.} \\
 & \multicolumn{1}{|p{11cm}|}{For p$_1$$>$10, the number of VFO steps is multiplied with} \\
 & \multicolumn{1}{|p{11cm}|}{a speed-up factor that increases up to 4 at p$_1$=30} \\
 & \multicolumn{1}{|p{11cm}|}{(corresponds to 3 turns of the VFO dial per second).} \\
 & \multicolumn{1}{|p{11cm}|}{This implements an over-proportional tuning speed if} \\
 & \multicolumn{1}{|p{11cm}|}{turning the VFO knob faster and faster.} \\
\bottomrule
\end{tabular}
\end{center}
...
$\phantom{\Big|}${\large Notes} & \multicolumn{1}{|p{11cm}|}{ANDROMEDA extension.} \\
 ...
 & \multicolumn{1}{|p{11cm}|}{The type (p$_1$) sent by a client does affect the} \\
 & \multicolumn{1}{|p{11cm}|}{processing of ZZZE and ZZZP commands from that client.} \\
 & \multicolumn{1}{|p{11cm}|}{Only the cases p$_1$=1 (original ANDROMEDA console)} \\
 & \multicolumn{1}{|p{11cm}|}{and p$_1$=5 (G2V2 console) are implemented.} \\
\bottomrule
\end{tabular}
\end{center}
...
For \texttt{Controller2}, there are not enough ,,spare'' GPIO lines to fully
...
when using \pH's internal iambic keyer, and use \bltt{PTT (keyer)} and
...
TCP is the method of choice for such a connection. \pH listens on port 19090 (by default,
...
via a null modem. Then use the serial port both in \pH (\bltt{RigCtl} menu) and the
digi mode program.
...
If you create this file, it should lines of the form
...
This option enables \pH to communicate with radios through the SoapySDR library.
}
...
If you have a RedPitay and no autostart of the SDR application there, and not compiled \pH
...
\item[\rett{AUDIO}] {(default: \texttt{PULSE})
...
which has not yet been found and fixed. Currently, the development version has version number 2.4.
...
\vskip0.2cm
\grtt{git checkout Rel-2.3}

\vskip0.2cm
and this will then switch your local repository to the release version 2.3. No more details on how
...
In principle, one can have a DHCP server running on the computer that also runs \pH, and thi8
