...
If piHPSDR is compiles without GPIO support, you can only choose between \rett{No Controller} and
\rett{G2 Mk2 Panel} (the latter is controlled by a serial connection rather than by GPIO lines).
The effect of choosing \rett{G2 Mk2 Panel} is that piHPSDR will then use a default when configuring
...
and the
IP address of the computer running piHPSDR (here overwritten with yyy.yyy.yyy.yyy). The IP and MAC address of
...
Although piHPSDR allows you to play with it, this option should nearly always be
...
such that piHPSDR shapes the side tone.
...
piHPSDR from other programs or even other computers. You can have up to three simultaneous CAT connections
via TCP, and three additional CAT connections via serial lines (provided the host computer running piHPSDR
...
\rett{Serial}. piHPSDR support up two three CAT connections over serial lines or named pipes.
...
the props file.
 It is important to know that piHPSDR does not try to ,,detect'' serial
...
of piHPSDR. Note that the \rett{Andromeda} flag is only inspected when the connection is opened. Serial CAT connections
...
piHPSDR has built-in support for making operating easier for those with very limited
...
Integrating a text-to-speech functionality inside piHPSDR is beyond the scope of the program,
but there are programs around, e.g. \texttt{eSpeak} or \texttt{festi\-val},
 that one can use. The ,,hook'' implemented
in piHPSDR is as follows (now it becomes technical): a message that is to be read aloud is
...
is included in the piHPSDR source code tree (\texttt{udplistener.c}) for demonstration purposes.
This program can either use the \texttt{eSpeak} or \texttt{festival} programs.
The function key events implemented are
...
Note there may be strong fluctuation in the reported value.

\rett{F5} Hitting the F5 function key reads the value of the TX drive slider.

\rett{F6} Hitting the F6 function key reads the gain or the attenuation in the front
...
{\color{red} Note that no GPIO lines are accessible and available if piHPSDR runs inside the Saturn G2 radios.}
...
If you need the piHPSDR built-in serial port, activate the \rett{Serial Port} button and deactivate
